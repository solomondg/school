...
vector<int> getResponses() { // Will get the responses from user, return as int array
    ...
    cout << "Welcome to Solomon Greenberg's Fare Calculator" << endl; // Part of the code that prompts users for all the variables we need to compute the fares
...
vector<float> fareCalc(vector<int> responseInput) { // Computes fares and fuel charge
    float fare = 0;
    float fuelCharge = 0;
...
int fareResponse(vector<float> fareAndFuel) { // Displays the fares + fuel charges
...
int returnToContinue(int blanklines) { // Cross-platform system("pause") thing since it's different on Unix
    int x;
